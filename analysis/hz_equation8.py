...
def fe_calc(file_list, n_bins, k=2000):
    ...
    for index,value in enumerate(file_list):
        ...
        for j in range(n_walkers):
            ...
            work_values[j] = np.array(wepy_h5.h5['runs/0/trajectories/'+str(j)+'/activity']).reshape((n_cycles))
            weights[j] = np.array(wepy_h5.h5['runs/0/trajectories/'+str(j)+'/weights']).reshape((n_cycles))
