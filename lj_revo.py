...
from resampling.resamplers.novelty_revo import NoveltyREVOResampler
from novelty.weight_factor import Weight_Factor_Novelty
...
if sys.argv[1] == "-h" or sys.argv[1] == "--help":
    print("arguments: n_cycles, n_steps, n_walkers, hdf5_filename, epsilon, out_dir")
else:
    ...
    char_dist = float(sys.argv[6]) # required for REVO simulations
    
# the inputs directory
inputs_dir = osp.realpath('./inputs')
# the outputs path
outputs_dir = osp.realpath(f'./outputs/resampler/{epsilon}_eps/')
...
## Resampler
novelty = Weight_Factor_Novelty(n_walkers)
resampler = NoveltyREVOResampler(distance=distance, char_dist=char_dist, init_state=init_state, merge_dist=2.5, novelty=novelty)
